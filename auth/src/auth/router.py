...
async def forgot_password(user_email: EmailStr):
    ...
    """
    await AuthService(AuthRepository).forgot_password(email=user_email)
    return {
...
async def reset_password(token: str, new_password: constr(min_length=8)):
    ...
    """
    await AuthService(AuthRepository).reset_password(token=token, new_password=new_password)
...
async def delete_my_account(user_credentials: UserTokenPayloadSchema = Depends(AuthDependency.get_current_user)):
    ...
    """
    return await AuthService(AuthRepository).delete_my_account(user_credentials.sub)
...
async def unban_user_by_email(
    ...
    await AuthService(AuthRepository).unban_user_by_email(email_for_unban=user_email, producer=user_credentials)
    return {
