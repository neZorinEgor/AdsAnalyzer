...
class ADSInfoRepository(IADSRepository):
    @staticmethod
    async def get_ads_report_paginate(limit: int, offset: int, user_email: str):
        async with session_factory() as session:
            ...
            return [AdsReportInfoSchema(**i.__dict__) for i in ads_report_user] if ads_report_user else  None
    ...
    @staticmethod
    async def get_report_info_by_id(report_id: int, user_email: str) -> Optional[AdsReportInfoSchema]:
        async with session_factory() as session:
